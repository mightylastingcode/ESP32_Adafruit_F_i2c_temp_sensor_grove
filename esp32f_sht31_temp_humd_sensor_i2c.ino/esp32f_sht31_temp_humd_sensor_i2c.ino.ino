...
// ver 16 change word to uint16_t for ESP32F
...
const uint16_t command_statusread  = 0xF32D;  // read register
const uint16_t command_statusclear = 0x3041;  // clear register
const uint16_t command_softreset   = 0x30A2;  // generate a reset
const uint16_t command_heaton      = 0x306D;  // enable heater
const uint16_t command_heatoff     = 0x3066;  // disable heater
const uint16_t command_startmeasL  = 0x2416;  // Start single shot measurement with 4ms sensing duration
const uint16_t command_startmeasM  = 0x240B;  // Start single shot measurement with 4ms sensing duration
const uint16_t command_startmeasH  = 0x2400;  // Start single shot measurement with 4ms sensing duration
const uint16_t command_break       = 0x3093;  // Break the current sensing, so another new command can be issued.
...
byte write2slave (int addr, uint16_t command);
...
void loop() {
  ...
  do {
      ...
      if (flag) {
        ...
        temperature_raw = uint16_t(datain[0]) << 8;
        temperature_raw = temperature_raw + uint16_t(datain[1]);
        ...
        humidity_raw    = uint16_t(datain[3]) << 8;
        humidity_raw    = humidity_raw + uint16_t(datain[4]);
